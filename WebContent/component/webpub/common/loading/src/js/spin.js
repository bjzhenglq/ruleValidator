define(function(require, exports, module) {
	...
	!function(window, document, undefined) {
		...
		/**
		 * Copyright (c) 2011 Felix Gnass [fgnass at neteye dot de] Licensed
		...
		function addAnimation(alpha, trail, i, lines) {
			...
			if (!animations[name]) {
				sheet.insertRule('@' + pre + 'keyframes ' + name + '{'
								+ '0%{opacity:' + z + '}' + start
								+ '%{opacity:' + alpha + '}' + (start + 0.01)
								+ '%{opacity:1}' + (start + trail) % 100
								+ '%{opacity:' + alpha + '}' + '100%{opacity:'
								+ z + '}' + '}', sheet.cssRules.length)
		...
		merge(Spinner.prototype, {
			spin : function(target) {
				...
				if (!useCssAnimations) {
					...
					var i = 0, fps = o.fps, f = fps / o.speed, ostep = (1 - o.opacity)
		...
		(function anim() {
						...
						for (var s = o.lines; s; s--) {
							var alpha = Math.max(1 - (i + s * astep) % f
											* ostep, o.opacity)
				...
				function fill(color, shadow) {
					return css(createEl(), {
								...
								transform : 'rotate('
										...
										+ ',0)',
								borderRadius : (o.corners * o.width >> 1)
										+ 'px'
							})
				}
				...
				for (; i < o.lines; i++) {
					seg = css(createEl(), {
								...
								animation : useCssAnimations
										&& addAnimation(o.opacity, o.trail, i,
												o.lines) + ' ' + 1 / o.speed
										+ 's linear infinite'
							})
