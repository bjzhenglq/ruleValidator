...
// Parses the default mime types string into a mimes lookup map
			...
			var plupload = {
				...
				cleanName : function(name) {
					...
					// Replace diacritics
					lookup = [/[\300-\306]/g, 'A', /[\340-\346]/g, 'a',
							/\307/g, 'C', /\347/g, 'c', /[\310-\313]/g, 'E',
							/[\350-\353]/g, 'e', /[\314-\317]/g, 'I',
							/[\354-\357]/g, 'i', /\321/g, 'N', /\361/g, 'n',
				...
				guid : function() {
					...
					return (plupload.guidPrefix || 'p') + guid
							+ (count++).toString(32);
				},
				...
				parseSize : function(size) {
					...
					if (typeof(size) == 'string') {
						size = /^([0-9]+)([mgk]?)$/.exec(size.toLowerCase()
								.replace(/[^0-9mkg]/g, ''));
						mul = size[2];
						size = +size[1];
				...
				xmlEncode : function(str) {
					return str ? ('' + str).replace(xmlEncodeRegExp, function(
									chr) {
								return xmlEncodeChars[chr] ? '&'
										+ xmlEncodeChars[chr] + ';' : chr;
							}) : str;
				},
				...
				removeClass : function(obj, name) {
					...
					obj.className = obj.className.replace(regExp, function($0,
									$1, $2) {
								return $1 === ' ' && $2 === ' ' ? ' ' : '';
							});
				},
				...
				addEvent : function(obj, name, callback) {
					...
					events[name].push({
								func : func,
								orig : callback, // store original callback
								// for IE
								key : key
							});
				},
					...
					for (var i = type.length - 1; i >= 0; i--) {
						...
						if (type[i].key === key || type[i].orig === callback) {
							...
							if (obj.removeEventListener) {
								obj.removeEventListener(name, type[i].func,
										false);
			...
			plupload.Uploader = function(settings) {
				...
				function calc() {
					...
					if (total.size === undef) {
						total.percent = files.length > 0 ? Math
								.ceil(total.uploaded / files.length * 100) : 0;
					} else {
				...
				plupload.extend(this, {
					...
					init : function() {
						...
						if (typeof(settings.preinit) == "function") {
							...
							plupload.each(settings.preinit,
									function(func, name) {
										self.bind(name, func);
									});
						}
						...
						self.bind('FilesAdded', function(up, selected_files) {
							...
							if (filters && filters.length) {
								...
								extensionsRegExp = new RegExp(extensionsRegExp
												.join('|')
												+ '$', 'i');
							}
							...
							for (i = 0; i < selected_files.length; i++) {
								...
								if (extensionsRegExp
										&& !extensionsRegExp.test(file.name)) {
									up.trigger('Error', {
							...
							if (count) {
								...
								return false; // Stop the FilesAdded event
								// from immediate propagation
							}
						});
						...
						self.bind('UploadProgress', function(up, file) {
									file.percent = file.size > 0
											? Math.ceil(file.loaded / file.size
													* 100)
											: 100;
									calc();
								});
					...
					splice : function(start, length) {
						...
						removed = files.splice(start === undef ? 0 : start,
			...
			plupload.File = function(id, name, size) {
				var self = this; // Setup alias for self to reduce code size
			...
			d.runtimes.Flash = d.addRuntime("flash", {
				...
				init : function(m, o) {
					...
					m.bind("Flash:Init", function() {
						var r = {}, q;
						try {
							n().setFileFilters(m.settings.filters,
						...
						m.bind("Flash:ExifData", function(s, t) {
									m.trigger("ExifData", m.getFile(r[t.id]),
											t.data)
								});
						m.bind("Flash:GpsData", function(s, t) {
									m.trigger("GpsData", m.getFile(r[t.id]),
											t.data)
								});
						...
						m.bind("Refresh", function(s) {
							...
							if (t) {
								u = d.getPos(t,
										b.getElementById(s.settings.container));
								v = d.getSize(t);
								d.extend(b.getElementById(s.id
				...
				init : function(g, i) {
					...
					function b() {
						...
						g.bind("PostInit", function() {
							var n, l = d.drop_element, p = g.id + "_droptarget", k = document
									.getElementById(l), m;
							function q(s, r) {
		...
		(function(window, document, plupload, undef) {
			...
			plupload.runtimes.Html4 = plupload.addRuntime("html4", {
				...
				init : function(uploader, callback) {
					uploader.bind("Init", function(up) {
						var container = document.body, iframe, url = "javascript", currentFile, input, currentFileId, fileIds = [], IE = /MSIE/
								.test(navigator.userAgent), mimes = [], filters = up.settings.filters, i, ext, type, y;
						...
						function createForm() {
							...
							if (up.features.triggerDialog && browseButton) {
								plupload.addEvent(
										getById(up.settings.browse_button),
										'click', function(e) {
											if (!input.disabled) {
												input.click();
											}
							...
							plupload.addEvent(input, 'change', function(e) {
								...
								if (element.value) {
									getById('form_' + currentFileId).style.top = -0xFFFFF
											+ "px";
									...
									// get fullpath
									var fullpath = name;
									name = name.substring(name.length, name
													.lastIndexOf('/')
													+ 1);
									// Push files
									var file = new plupload.File(currentFileId,
											name);
									...
									if (!up.features.triggerDialog) {
										...
										plupload.removeEvent(browseButton,
												'click', up.id);
									}
									plupload
											.removeEvent(input, 'change', up.id);
						...
						function createIframe() {
							...
							plupload.addEvent(iframe, 'load', function(e) {
								...
								try {
									el = n.contentWindow.document
											|| n.contentDocument
											|| window.frames[n.id].document;
								} catch (ex) {
						...
						up.bind('FileUploaded', function(up) {
									up.refresh(); // just to get the form back
													// on top of browse_button
								});
						...
						up.bind("Refresh", function(up) {
							...
							if (browseButton) {
								browsePos = plupload.getPos(browseButton,
										getById(up.settings.container));
								browseSize = plupload.getSize(browseButton);
								inputContainer = getById('form_'
										+ currentFileId);
								...
								// feature will change
								if (up.features.triggerDialog) {
									if (plupload.getStyle(browseButton,
											'position') === 'static') {
								...
								topElement = up.features.triggerDialog
										? browseButton
										: inputContainer;

								if (hoverClass) {
									plupload.addEvent(topElement, 'mouseover',
											function() {
												plupload.addClass(browseButton,
														hoverClass);
											}, up.id);
									plupload.addEvent(topElement, 'mouseout',
											function() {
								...
								if (activeClass) {
									plupload.addEvent(topElement, 'mousedown',
											function() {
												plupload.addClass(browseButton,
														activeClass);
											}, up.id);
									plupload.addEvent(document.body, 'mouseup',
											function() {
												plupload.removeClass(
														browseButton,
														activeClass);
