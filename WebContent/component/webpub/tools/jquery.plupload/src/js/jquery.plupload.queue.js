define(function(require, exports, module) {
	return function(jQuery) {
		var jQuery = require("./plupload")(jQuery);
/**
				...
				if (settings) {
					this.each(function() {
						...
						function updateTotalProgress() {
							$('span.plupload_total_status', target)
									.html(uploader.total.percent + '%');
							$('div.plupload_progress_bar', target).css('width',
									uploader.total.percent + '%');
							$('span.plupload_upload_status', target)
						...
						function updateList() {
							...
							$('span.plupload_total_file_size', target)
									.html(plupload
						...
						uploader.bind('Init', function(up, res) {
							...
							if (!settings.unique_names && settings.rename) {
								$(
									...
									// extension
									file = up
											.getFile(targetSpan.parents('li')[0].id);
									name = file.name;
									parts = /^(.+)(\.[^.]+)$/.exec(name);
									if (parts) {
										name = parts[1];
