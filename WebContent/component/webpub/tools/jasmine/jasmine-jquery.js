define('jasmine-jquery', ['$'], function(require) {
	...
	jasmine.Fixtures.prototype.addToContainer_ = function(html) {
		var container = jQuery('body').find('#' + this.containerId)
				.append(html)
		if (!container.length) {
			this.createContainer_(html)
		}
	...
	jasmine.Fixtures.prototype.proxyCallTo_ = function(methodName,
			passedArguments) {
	...
	jasmine.JSONFixtures.prototype.loadFixtureIntoCache_ = function(relativeUrl) {
		var self = this
		var url = this.fixturesPath.match('/$') ? this.fixturesPath
				+ relativeUrl : this.fixturesPath + '/' + relativeUrl
		jQuery.ajax({
			...
			spyOn : function(selector, eventName) {
				var handler = function(e) {
					data.spiedEvents[jasmine
							.spiedEventsKey(selector, eventName)] = e
				}
	...
	!function() {
		...
		var bindMatcher = function(methodName) {
			...
			jasmine.JQuery.matchersClass[methodName] = function() {
				if (this.actual
						&& (this.actual instanceof jQuery || jasmine
								.isDomNode(this.actual))) {
					this.actual = $(this.actual)
					var result = jQueryMatchers[methodName].apply(this,
							arguments)
					var element
					if (this.actual.get && (element = this.actual.get()[0])
							&& !$.isWindow(element)
							&& element.tagName !== "HTML")
						this.actual = jasmine.JQuery
		...
		this.addMatchers({
			toHaveBeenPrevented : function() {
				...
				this.message = function() {
					return [
							"Expected event " + eventName
									+ " to have been prevented on " + selector,
							"Expected event " + eventName
									+ " not to have been prevented on "
									+ selector]
				}
