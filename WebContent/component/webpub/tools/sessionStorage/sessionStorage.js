 ...
 * 
 * @description sessionStorage object is an HTML5 working draft which aim is to
 *              remove cookie limits. Using a sort of "de-facto" standard as
 *              window.name behavior is, it is possible to implement the
 *              sessionStorage object in old browsers. The concept is that
 *              window.name persists in the tab, whatever page we are surfing,
 *              while as security measure I combined browser cookie management
 *              plus RC4 stream cipher in order to reduce security problems as
 *              much as possible.
 * 
 ...
 * @version 1.4
 * @compatibility Android, Chrome, Explorer, Opera, unobtrusive for other
 *                browsers
 * @credits W3 WebStorage Draft http://dev.w3.org/html5/webstorage/ RC4 Stream
 *          Cipher http://www.wisdom.weizmann.ac.il/~itsik/RC4/rc4.html
 * @protocol Linear String Storage, by the
 * @author
 */
...
// sessionStorage is a Singleton instance:
...
function sessionStorage() {
	...
	function clear() {
		...
		document.cookie = ["sessionStorage=" + window.encodeURIComponent($key = RC4.key(128))].join(';');
	...
	if (data) {
		...
		// loop over each key in order to assign the correct for this instance,
		// if any ...
...
if (cache !== null)
	// be sure both top context and this context (could be the same) point to
	// the same object
