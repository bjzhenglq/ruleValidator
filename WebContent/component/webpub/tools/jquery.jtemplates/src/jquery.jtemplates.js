...
define( function(require, exports, module) {
	return function(jQuery) {
			...
			( function(jQuery) {
				 ...
				 * @param {string}
				 *            s A template string (like: "Text: {$T.txt}.").
				 * @param {array}
				 *            [includes] Array of included templates.
				 * @param {object}
				 *            [settings] Settings.
				 * @config {boolean} [disallow_functions] Do not allow use
				 *         function in data (default: true).
				 * @config {boolean} [filter_data] Enable filter data using
				 *         escapeHTML (default: true).
				 * @config {boolean} [filter_params] Enable filter parameters
				 *         using escapeHTML (default: false).
				 * @config {boolean} [runnable_functions] Automatically run
				 *         function (from data) inside {} [default: false].
				 * @config {boolean} [clone_data] Clone input data [default:
				 *         true]
				 * @config {boolean} [clone_params] Clone input parameters
				 *         [default: true]
				 * @config {Function} [f_cloneData] Function used to data
				 *         cloning
				 * @config {Function} [f_escapeString] Function used to escape
				 *         strings
				...
				var Template = function(s, includes, settings) {
					...
					// set handlers
					this.f_cloneData = (this.settings.f_cloneData !== undefined) ? (this.settings.f_cloneData)
							: (TemplateUtils.cloneData);
					this.f_escapeString = (this.settings.f_escapeString !== undefined) ? (this.settings.f_escapeString)
							: (TemplateUtils.escapeHTML);
					this.f_parseJSON = (this.settings.f_parseJSON !== undefined) ? (this.settings.f_parseJSON)
							: ((this.settings.disallow_functions) ? (jQuery.parseJSON)
									: (TemplateUtils.parseJSON));
					...
					if (s) {
						// set main template
						this.setTemplate(this._templates_code['MAIN'],
								includes, this.settings);
					}
				...
				Template.prototype.splitTemplates = function(s, includes) {
					...
					while ((iter = reg.exec(s)) != null) {
						...
						if (se == -1) {
							throw new Error(
									'jTemplates: Template "' + tname + '" is not closed.');
						}
						// save a subtemplate and parse options
						this._templates_code[tname] = s
								.substring(lastIndex, se);
						_template_settings[tname] = TemplateUtils
								.optionToObject(iter[2]);
					}
					...
					for ( var i in this._templates_code) {
						if (i != 'MAIN') {
							this._templates[i].setTemplate(
									this._templates_code[i], jQuery.extend( {},
											includes || {}, this._templates
													|| {}), jQuery.extend( {},
											this.settings,
											_template_settings[i]));
				...
				Template.prototype.setTemplate = function(s, includes, settings) {
					...
					this._includes = jQuery.extend( {}, this._templates || {},
							includes || {});
					...
					for ( var i = 0, l = (op) ? (op.length) : (0); i < l; ++i) {
						...
						if (se > ss) {
							node.push(new TextNode(s.substring(ss, se),
									literalMode, this));
						}
						var ppp = this_op.match(/\{#([\w\/]+).*?\}/); // find
																		// operator
																		// name
						var op_ = RegExp.$1;
						switch (op_) {
						...
						case 'include':
							node
									.push(new Include(this_op, this._includes,
											this));
							break;
						case 'param':
						...
						case '/literal':
							if (Template.DEBUG_MODE) {
								throw new Error(
										"jTemplates: Missing begin of literal.");
							}
							break;
						default:
							if (Template.DEBUG_MODE) {
								throw new Error(
										'jTemplates: unknown tag: ' + op_ + '.');
							}
						}
					...
					if (s.length > ss) {
						node
								.push(new TextNode(s.substr(ss), literalMode,
										this));
					}
				};
				...
				/**
				 * Replace chars &, >, <, ", ' with html entities. To disable
				 * function set settings: filter_data=false, filter_params=false
				 * 
				 * @param {string}
				 *            string
				 * @return {string}
				...
				/**
				 * Make a copy od data 'd'. It also filters data (depend on
				 * 'filter').
				 * 
				 * @param {object}
				 *            d input data
				 * @param {object}
				 *            filter a filters
				 * @config {boolean} [escapeData] Use escapeHTML on every
				 *         string.
				 * @config {boolean} [noFunc] Do not allow to use function
				 *         (throws exception).
				 * @param {Function}
				 *            f_escapeString function using to filter string
				 *            (usually: TemplateUtils.escapeHTML)
				...
				TemplateUtils.cloneData = function(d, filter, f_escapeString) {
					...
					if (d == null) {
						...
						// return d;
						// after modify
						// hajack for null,convert null to empty string "";
						// by mengzha
						return "";
					}
					...
					case Function:
						if (filter.noFunc) {
							if (Template.DEBUG_MODE)
								throw new Error(
										"jTemplates: Functions are not allowed.");
							else
								return undefined;
						}
					}
				 ...
				 * 
				 * @param {string}
				 *            optionText text-based option string
				 * @return {Object}
				 * @static
				 * @memberOf TemplateUtils
				 */
				 ...
				 *            el html element
				 * @param {int}
				 *            guid template process unique identificator
				 * @param {int}
				 *            id index
				 * @return {object}
				 * @static
				 ...
				 *            val text string
				 * @param {boolean}
				 *            literalMode When enable (true) template does not
				 *            process blocks {..}.
				 * @param {Template}
				 *            Template object
				 * @augments BaseNode
				...
				TextNode.prototype.get = function(d, param, element, deep) {
					...
					while (true) {
						...
						if ((lm != -1 && lm < rm) || (rm == -1)) {
							...
							if (--nested === 0) {
								if (sText >= 0) {
									result += this._template.getBin()
											.evaluateContent(d, param, element,
													s.substring(sText, rm + 1));
									sText = -1;
									sExpr = i + 1;
								}
							} else if (nested < 0) {
				 ...
				 *            $Q element
				 * @param {String}
				 *            __value Template content
				 * @return {String}
				 */
				EvalClass.prototype.evaluateContent = function($T, $P, $Q,
						__value) {
					try {
						var result = eval(__value);

						if (jQuery.isFunction(result)) {
							if (this.__templ.settings.disallow_functions
									|| !this.__templ.settings.runnable_functions) {
								return '';
							}
							result = result($T, $P, $Q);
						}
				...
				opIF.prototype.get = function(d, param, element, deep) {
					...
					try {
						var arr = (this._templ.getBin().evaluate(d, param,
								element, this._cond)) ? (this._onTrue)
								: (this._onFalse);
				...
				/**
				 * Handler for a tag 'FOR'. Create new and return relative
				 * opFOREACH object.
				 * 
				 * @name opFORFactory
				 * @class Handler for a tag 'FOR'. Create new and return
				 *        relative opFOREACH object.
				 * @param {string}
				 *            oper content of operator {#..}
				 * @param {object}
				 *            par parent node
				 * @param {Template}
				 *            template a pointer to Template object
				 * @return {opFOREACH}
				 */
				opFORFactory = function(oper, par, template) {
					// create operator FOREACH with function as iterator
					if (oper
							.match(/\{#for (\w+?) *= *(\S+?) +to +(\S+?) *(?:step=(\S+?))*\}/)) {
						...
						throw new Error(
								'jTemplates: Operator failed "find": ' + oper);
					}
				};
				...
				var opFOREACH = function(oper, par, template) {
					...
					if (oper != null) {
						...
						this._option = TemplateUtils
								.optionToObject(this._option);
					}
				...
				opFOREACH.prototype.get = function(d, param, element, deep) {
					try {
						...
						var fcount = (this._runFunc === undefined) ? (this._template
								.getBin()
								.evaluate(d, param, element, this._arg))
								: (this._runFunc);
						if (fcount === $) {
							throw new Error(
									"jTemplate: Variable '$' cannot be used as loop-function");
						}
						...
						// setup primary iterator, iterator can get data from
						// options (using by operator FOR) or from data "$T"
						var extData = (this._option.extData !== undefined) ? (this._template
								.getBin().evaluate(d, param, element,
								this._option.extData))
								: ((d != null) ? (d) : ( {}));
						if (extData == null) {
							extData = {};
						}
						// start, end and step
						var s = Number(this._template.getBin().evaluate(d,
								param, element, this._option.begin) || 0), e; // start,
																				// end
						var step = Number(this._template.getBin().evaluate(d,
								param, element, this._option.step) || 1);
						if (mode != 'function') {
							e = fcount.length;
						} else {
							if (this._option.end === undefined
									|| this._option.end === null) {
								e = Number.MAX_VALUE;
							} else {
								e = Number(this._template.getBin().evaluate(d,
										param, element, this._option.end))
										+ ((step > 0) ? (1) : (-1));
							}
						}
						...
						if (this._option.count) {
							...
							var tmp = s
									+ Number(this._template.getBin().evaluate(
											d, param, element,
											this._option.count));
							e = (tmp > e) ? (e) : (tmp);
						}
						...
						if ((e > s && step > 0) || (e < s && step < 0)) {
							var iteration = 0;
							var _total = (mode != 'function') ? (Math
									.ceil((e - s) / step)) : undefined;
							var ckey, cval; // current key, current value
							var loopCounter = 0;
							for (; ((step > 0) ? (s < e) : (s > e)); s += step, ++iteration, ++loopCounter) {
								if (Template.DEBUG_MODE
										&& loopCounter > Template.FOREACH_LOOP_LIMIT) {
									throw new Error(
											"jTemplate: Foreach loop limit was exceed");
								}
								ckey = key[s];
								if (mode != 'function') {
									...
									// if no result from function then stop
									// foreach
								...
								if ((typeof cval == 'function')
										&& (this._template.settings.disallow_functions || !this._template.settings.runnable_functions)) {
									continue;
								}
								...
								extData[this._name + '$key'] = (ckey !== undefined && ckey.constructor == String) ? (this._template
										.f_escapeString(ckey))
										: (ckey);
								extData[this._name + '$typeof'] = typeof cval;
								for (i = 0, l = this._onTrue.length; i < l; ++i) {
									try {
										ret += this._onTrue[i].get(extData,
												param, element, deep);
									} catch (ex) {
										if (ex instanceof JTException) {
											switch (ex.type) {
											case 'continue':
												i = l; // force skip to next
														// node
												break;
											case 'break':
												i = l; // force skip to next
														// node
												s = e; // force skip outsite
														// foreach
												break;
											default:
												throw ex;
							...
							for (i = 0, l = this._onFalse.length; i < l; ++i) {
								ret += this._onFalse[i].get(d, param, element,
										deep);
							}
						}
						return ret;
				 ...
				 * 
				 * @name JTException
				 * @class A class used internals for a template-control
				 *        exceptions
				 * @param type
				 *            {string} Type of exception
				 * @augments Error
				 * @augments BaseNode
				 */
				...
				var Include = function(oper, includes, templ) {
					...
					if (this._template == undefined) {
						if (Template.DEBUG_MODE)
							throw new Error(
									'jTemplates: Cannot find include: ' + RegExp.$1);
					}
					this._root = RegExp.$2;
				...
				Include.prototype.get = function(d, param, element, deep) {
					try {
						...
						return this._template.get(this._mainTempl.getBin()
								.evaluate(d, param, element, this._root),
								param, element, deep);
					} catch (e) {
						if (Template.DEBUG_MODE || (e instanceof JTException)) {
							throw e;
				...
				UserParam.prototype.get = function(d, param, element, deep) {
					try {
						param[this._name] = this._templ.getBin().evaluate(d,
								param, element, this._value);
					} catch (e) {
						if (Template.DEBUG_MODE || (e instanceof JTException)) {
							throw e;
						}
				 ...
				 * @param {Number}
				 *            deep
				 * @return {String} calling of function ReturnRefValue (as text
				 *         string)
				 */
				UserVariable.prototype.get = function(d, param, element, deep) {
					try {
						...
						var obj = this._templ.getBin().evaluate(d, param,
								element, this._id);
						...
						return "(TemplateUtils.ReturnRefValue(this,"
								+ refobj.guid + "," + (i - 1) + "))";
					} catch (e) {
						if (Template.DEBUG_MODE || (e instanceof JTException)) {
							throw e;
				 ...
				 *            s a Template or a template string
				 * @param {array}
				 *            [includes] Array of included templates.
				 * @param {object}
				 *            [settings] Settings (see Template)
				 ...
				 * @memberOf jQuery.fn
				 */
				jQuery.fn.setTemplateElement = function(elementName, includes,
						settings) {
				...
				/**
				 * Check it HTML Elements have a template. Return count of
				 * templates.
				 * 
							...
							.each(
									function() {
										...
										if (t === undefined) {
											if (Template.DEBUG_MODE)
												throw new Error(
														'jTemplates: Template is not defined.');
											else
												return;
										}
					...
					return jQuery(this)
							.each(
									function() {
										...
										jQuery.data(this, 'jTemplateSID',
												jQuery.data(this,
														'jTemplateSID') + 1);
										jQuery(this).html(
												t.get(d, param, this, 0));
									});
				};

				/**
				 * Process template using data from URL 'url_' (only format
				 * JSON) and parameters 'param'. Update HTML code.
				 * 
				 * @param {string}
				 *            url_ URL to data (in JSON)
				 * @param {object}
				 *            [param] parameters
				 * @param {object}
				 *            options options (over ajaxSettings) and callbacks
				 * @return {jQuery} chainable jQuery class
				 * @memberOf jQuery.fn
				 */
				 ...
				 * @param {number}
				 *            interval Time refresh interval
				 * @param {object}
				 *            args Additional URL parameters (in URL alter ?) as
				 *            assoc array.
				 * @param {array}
				 *            objs An array of HTMLElement which will be
				 *            modified by Updater.
				 * @param {object}
				 *            options options and callbacks
				 */
				...
				/**
				 * Create new HTTP request to server, get data (as JSON) and
				 * send it to templates. Also check does HTMLElements still
				 * exists in Document.
				 */
				Updater.prototype.run = function() {
					...
					jQuery.ajax( {
						...
						success : function(d) {
							try {
								var r = jQuery(that.objs).processTemplate(d,
										that._param, {
											StrToJSON :true
										});
				...
				/**
				 * Check does HTMLElements still exists in HTML Document. If not
				 * exist, delete it from property 'objs'.
				 */
				 ...
				 * @memberOf jQuery.fn
				 */
				jQuery.fn.processTemplateStart = function(url, param, interval,
						args, options) {
					return new Updater(url, param, interval, args, this,
							options);
				};
				...
				jQuery
						.extend(/** @scope jQuery.prototype */
						{
							/**
							 * Create new Template.
							 * 
							 * @param {string}
							 *            s A template string (like: "Text:
							 *            {$T.txt}.").
							 * @param {array}
							 *            includes Array of included templates.
							 * @param {object}
							 ...
							 * @return {Template}
							 */
							createTemplateURL : function(url_, includes,
									settings) {
								var s = jQuery.ajax( {
									url :url_,
									dataType :'text',
							 ...
							 */
							processTemplateToText : function(template, data,
									parameter) {
								return template.get(data, parameter, undefined,
										0);
							},
