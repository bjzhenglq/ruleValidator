	...
	(function() {
		...
		_.map = _.collect = function(obj, iterator, context) {
			...
			each(obj, function(value, index, list) {
						results[results.length] = iterator.call(context, value,
								index, list);
					});
			return results;
		};
		...
		_.reduceRight = _.foldr = function(obj, iterator, memo, context) {
			...
			each(obj, function(value, index, list) {
						...
						if (!initial) {
							...
							memo = iterator.call(context, memo, obj[index],
									index, list);
						}
					});
			if (!initial)
		...
		var flatten = function(input, shallow, output) {
			each(input, function(value) {
						if (_.isArray(value)) {
							shallow ? push.apply(output, value) : flatten(
									value, shallow, output);
						} else {
							output.push(value);
						}
					});
		...
		// **MSIE**),
		// we need this function. Return the position of the first occurrence of
		// an
		...
		_.memoize = function(func, hasher) {
			...
			return function() {
				...
				return _.has(memo, key) ? memo[key] : (memo[key] = func.apply(
						this, arguments));
			};
		};
		...
		_.template = function(text, data, settings) {
			...
			try {
				var render = new Function(settings.variable || 'obj', '_',
						source);
			} catch (e) {
				e.source = source;
				throw e;
