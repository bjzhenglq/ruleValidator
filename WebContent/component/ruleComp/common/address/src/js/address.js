define(function(require, exports, module) {
	 ...
	 */
	function addAddress(grid,dom) {
		var that = dom;
		//行政区划
		var contentHtml = "<iframe id='addressEdit' data-resize='true' src='"+G.PAGE.USER_EDIT+"' frameborder='0' scrolling='no' width='450px' height='200px'></iframe>";
		Dialog.dialog({
			title : '新增用户',
			hasCloseTip : true,
			content : contentHtml,
	 ...
	 * 修改地址
	 */
	function updateAddress(record, grid) {
		//获取主键
		var pk = record.pkUser||"";
		var contentHtml = "<iframe id='addressEdit' data-resize='true' frameborder='0' scrolling='no' src='"+G.PAGE.USER_EDIT+"?pkUser="+pk+"' width='450px' height='250px'></iframe>";
		Dialog.dialog({
			title : '修改用户信息',
			hasCloseTip : true,
			content : contentHtml,
	 ...
	 * 删除用户
	 */
	function deleteAddress(record, grid) {
		// [0] 获取主键
		var dataModel = {};
