...
define(function(require, exports, module) {
	...
	var OrderCore = function() {
		...
		var orderAddressAdd = function(flag) {
			...
			// 收货地址
			var addressSearcher = $(".addressSearcher").eq(0).clone();
			// 联系人
			...
			var orderItemLine = $('<tr></tr>').append($('<td></td>').html("商品编码")).append($('<td></td>').html("商品名称")).append($('<td></td>').html("物料"))
					.append($('<td></td>').html("销售组织")).append($('<td></td>').html("价格")).append($('<td></td>').html("数量")/* .addClass("w_150px") */).append(
							$('<td></td>').html("库存")).append($('<td></td>').html("金额")).append($('<td></td>').html("操作"));
			// 退货
			if (flag && flag == "return") {
				...
				orderItemLine = $('<tr>').append($('<td>').html("商品编码")).append($('<td>').html("商品名称")).append($('<td>').html("物料")).append(
						$('<td>').html("销售组织")).append($('<td>').html("价格")).append($('<td>').html("数量").addClass("w_150px")).append($('<td>').html("金额"))
						.append($('<td>').html("退货原因")).append($('<td>').html("类型")).append($('<td>').html("操作"));
			}
			...
			var option = {
				...
				"addressSearcher" : addressSearcher,// 收货地址
			...
			// 初始化自动补全插件
			initAutoComplete($(this).parent(".order_btn_wrapper").prev());
			// 更新索引
			updateIndex();
			// 初始化日历控件
		...
		var generateAddressTemplate = function(data) {
			...
			var addressSearcher = data.addressSearcher;// 收货地址簿
			// 期望收获日期
			...
			if (data.isReturn) {
				...
				address_info = $('<div></div>').addClass("address_info").append($('<label></label>').html(customerAreaLabel)).append(
						$('<div></div>').append($('<select></select>').attr({

							name : "addressUIView[" + name_index + "].creceiveareaid"
						}).append(customerAreaOptions))).append($('<label></label>').attr({
					'for' : "address" + addressBars
				}).html(customerAddressLabel)).append($('<div></div>').append($('<select></select>').attr({

					name : "addressUIView[" + name_index + "].creceiveaddrid"
				}).append(customerAddressOptions))).append($('<label></label>').attr({
					'for' : "address" + addressBars
				}).html(dreceivedateLabel)).append(
						$('<div></div>').append($('<input data-datepicker-defaultDate="' + dreceivedate + '" style="width:120px;"/>').attr({
							name : "addressUIView[" + name_index + "].dreceivedate"
						}).addClass("textfiled datepicker").attr("data-datepicker-minDate", "0"))).append($('<label></label>').html("商品清单")).append(
						$('<div></div>').append($('<span></span>').addClass("color_red J-product-type").attr({
							id : "amound" + addressBars
						}).html(0)).append($('<span></span>').html(" 种")));

			} else {
				var creceiveaddrid_name = addressSearcher.find(".J-addressSearcher").val();
				address_info.append($('<label></label>').attr({
					'for' : "address" + addressBars
				}).html(customerAddressLabel)).append(
						addressSearcher.find("input[type='hidden']").attr("name", 'addressUIView[' + name_index + '].creceiveaddrid').val("").parent().find(
								".J-addressSearcher").attr({
							'value' : '',
							'title' : ''
						}).parent()).append($('<label></label>').html(customerAreaLabel)).append($('<div></div>').append($('<select></select>').attr({
					name : "addressUIView[" + name_index + "].creceiveareaid"
				}).append(customerAreaOptions))).append($('<label></label>').attr({
					'for' : "address" + addressBars
				}).html(dreceivedateLabel)).append(
			...
			if (orderItemLine == undefined) {
				orderItemLine = $('<tr></tr>').append($('<td></td>').html("商品编码")).append($('<td></td>').html("商品名称")).append($('<td></td>').html("物料"))
						.append($('<td></td>').html("销售组织")).append($('<td></td>').html("价格")).append($('<td></td>').html("数量").addClass("w_150px")).append(
								$('<td></td>').html("库存")).append($('<td></td>').html("金额")).append($('<td></td>').html("操作"));
			}
			// 地址详细商品列表
			...
			// 地址详细底部左部
			var list_btm_left = $('<div></div>').addClass("floatleft").append(
					$('<label></label>').append($('<input />').addClass("textfiled add_input_name").val("输入商品名称 / 编码"))).append(
					$('<label></label>').append($('<input />').addClass("textfiled add_input_amount").val(1))).append(
					$('<a href="#"></a>').addClass("btn_add").addClass(addProductMenuClass));
			// 从购物车添加
			if (showShopCart) {
			...
			var list_btm_right;
			if (isReturn) {
				var html = '<div class="list_total ui-order-total-price J-order-return-total-price-address-footer">'
						+ '<span class="ui-order-total-price-label">合计金额： </span>' + '<span class="ui-order-price-return-symbol">[退]</span>'
						+ '<span class="ui-order-price-return-number" scale="4">0</span>' + '<span class="ui-order-price-exchange-symbol">[换]</span>'
						+ '<span class="ui-order-price-exchange-number" scale="4">0</span>' + '</div>';
				list_btm_right = $(html);
			} else {
				list_btm_right = $('<div></div>').addClass("list_total font_yahei").append(
						$('<span></span>').addClass("total_title font_currency").html("合计金额： ")).append(
			...
			// 详细地址信息
			var idBarContentExtended = $('<div></div>').addClass("idBarContentExtended w_100ps mrg_b5px").append(address_info).append(product_table).append(
					list_btm);
			// 折叠起来的地址信息
			var info_line = '';
			if (isReturn) {
				addressHeaderTotalPrice = '<div class="w_300px ui-order-total-price J-order-return-total-price-address-collapse">'
						+ '<span class="ui-order-total-price-label">合计金额： </span>' + '<span class="ui-order-price-return-symbol">[退]</span>'
						+ '<span class="ui-order-price-return-number" scale="4">0</span>' + '<span class="ui-order-price-exchange-symbol">[换]</span>'
						+ '<span class="ui-order-price-exchange-number" scale="4">0</span>' + '</div>';
				info_line = $('<div></div>').addClass("info_line").append($('<a href="#"></a>').addClass("trigger up")).append(
						$('<div></div>').addClass("content_name font_bold").append(contentTitleLabel).append($("<span>").addClass("index").text(addressBars)))
						.append(
								$('<div></div>').addClass("content_info hidden").append(
										$('<div></div>').addClass("w_130px").append($('<span></span>').html("商品清单： ")).append(
												$('<span></span>').addClass("color_red J-product-type").attr({
													id : "amound" + addressBars
										...
										$('<div></div>').addClass("w_320px").attr({
											id : "address" + addressBars
												...
												$('<span></span>').addClass("creceiveaddrid_name").append(creceiveaddrid_name)))).append(
								$('<a href="#"></a>').addClass("icon_delete"));
			} else {
				...
				// </div>
				addressHeaderTotalPrice = '<div class="w_180px ui-order-total-price J-order-total-price-address-collapse">'
						+ '<span class="ui-order-total-price-label">合计金额： </span>' + '<span class="ui-order-price-number" scale="4">0</span>' + '</div>';
				info_line = $('<div></div>').addClass("info_line").append($('<a href="#"></a>').addClass("trigger up")).append(
						$('<div></div>').addClass("content_name font_bold").append(contentTitleLabel).append($("<span>").addClass("index").text(addressBars)))
						.append(
								$('<div></div>').addClass("content_info hidden").append(
			...
			// 地址栏
			var idAddressBar = $('<div></div>').addClass("rounded_extended_bar inside_width idAddressBar J-order-address").append(
					$('<div></div>').addClass("bar_corner lt")).append($('<div></div>').addClass("bar_border inside_width top")).append(
					$('<div></div>').addClass("bar_corner rt")).append(bar_content).append($('<div></div>').addClass("bar_corner lb")).append(
					$('<div></div>').addClass("bar_border inside_width btm")).append($('<div></div>').addClass("bar_corner rb"));
			return idAddressBar;
		};
		/**
		 * 从购物车添加
						...
						columns : [
								{
									...
									format : function(value, row) {
										return StringUtils.format('<a href="%s/html/product/product.html?id=%s" target="_blank" >%s</a>', [ G.ctx,
												row.cproductid, value ]);
									}
								},
								{
									label : "商品名称",
								...
								},
								{
									...
									align : 'right',
									format : function(value, row) {
										return StringUtils.format('<span class="ft_yahei">%s</span><span class="caclPrice" scale="%s">%s</span>', [
												row.corigcurrencyid_curSign, context.get("priceScale"), value ]);
									}
								},
								{
									label : "数量",
									...
									align : 'left',
									format : function(value, row) {
										return StringUtils.format('<input class="textfiled shopCart_caclSummy" value="%s" scale="%s"/><span>%s</span>', [
												row.count, row.pk_measdoc_unitScale, row.pk_measdoc_name ]);
									}
								},
								{
									label : "库存",
									...
									align : 'right',
									format : function(value, row) {
										return StringUtils.format('<span class="norigtaxmny" ectype="nabnum" unit="%s" productid="%s"scale="%s"></span>', [
												row.pk_measdoc_name, row.cproductid, row.pk_measdoc_unitScale ]);
									}
								},
								{
									label : "金额",
									...
									align : 'right',
									format : function(value, row) {
										return StringUtils.format('<span class="ft_yahei">%s</span><span class="norigtaxmny" scale="%s">%s</span>', [
												row.corigcurrencyid_curSign, row.corigcurrencyid_amountScale, row.price * row.count ]);
									}
								}, {
									label : "删除",
									key : "vcode",
		...
		var addProductItem = function(json, listBtm, panel, ordertype, callback) {
			...
			// 数量
			var productNum = $('<td class="align_left"></td>').append(
					$('<input />').addClass("textfiled order_nqtunitnum J_checkQtNum J-order-row-amount").val(amount).attr({
			...
			var productNnabnum = $('<td></td>').addClass("align_right validateNabnum").html(
					"<span ectype='nabnum'" + "productid='" + cproductid + "' scale='" + unitScale + "' unit='" + pk_measdoc_name + "'bspotflag='" + bspotflag
							+ "'></span>");
			...
			if (ordertype == "order") {
				...
				// 新增订单行
				$(newRow).append(productCode).append(productName).append(mateiralCode).append(saleOrgCode).append(productPrice).append(productNum).append(
						productNnabnum).append(productNtaxmny).append(hiddenInput).append(operation);
		...
		function setFieldName() {
			...
			var orderItemFieldNames = [ 'cproductid', 'cshopcartid', 'nqtunitnum', 'cmaterialid', 'pk_org', 'pk_org_name', 'corigcurrencyid', 'csrcbid',
					'cretreasonid', 'fretexchange', 'cexchangesrcretid', 'returnRowIndex', 'nqtorigtaxnetprc', 'nqtorigtaxprice_priceScale',
					'corigcurrencyid_curSign', 'vsrccode', 'csrcid', 'vsrcrowno', 'norigtaxmny', 'ntotaloutnum', 'ntotalreturnnum', 'ntotalsignnum',
					'cmaterialvid', 'ccustomerpoid', 'ccustomerpobid', 'nqtorignetprice', 'nqtorigprice', 'cqtunitid', 'blargessflag', 'taxRate',
		 ...
		 * 初始化自动补全商品,以及收货地址
		 */
		var initAutoComplete = function(DOM) {
			(DOM != null ? DOM.find(".add_input_name") : $('.add_input_name')).autocomplete(GLOBAL.API.PRODUCT_AUTOFILL, {
				max : 10, // 列表里的条目数
			...
			// 收货地址自动补全
			(DOM != null ? DOM.find(".J-addressSearcher") : $(".J-addressSearcher")).autocomplete(G.API.ACCOUNT_CUSTADDRESS_QUERY, {
				max : 50, // 列表里的条目数
				...
				autoFill : false, // 自动填充
				formatItem : function(row, i, max) {
					return StringUtils.format('%s \ %s', [ row.addressView.addressfullname, row.linkManView != null ? row.linkManView.name : "" ]);
				},
				formatMatch : function(row, i, max) {
					return row.pk_address;
				},
				formatResult : function(row) {
					return row.pk_address;
				},
				parse : function(data) {
					return $.map(eval(data), function(row) {
						return {
							...
							result : row.addressView.addressfullname
						};
					});
				}
			}).result(function(event, row, formatted) {
				var JDOM = DOM != null ? DOM.find(".J-addressSearcher") : $(this);
				var title = row.addressView.addressfullname;
				JDOM.parents(".address_info").find("select[name$='.creceiveareaid']").val(row.pk_areacl || "");
				JDOM.parent().find("input[type='hidden']").attr({
					value : row.pk_address
				});
				JDOM.attr({
					title : title.trim()
				});
				JDOM.parents(".idAddressBar").find(".creceiveaddrid_name").html(title.trim());
				JDOM.parent().find(".ui-cleaner").removeClass("hidden");
			}).focus(function(){
				$(this).parent().find(".ui-cleaner").removeClass("hidden");
			})/*.blur(function(){
				$(this).parent().find(".ui-cleaner").addClass("hidden");
			})*/;
		...
		var addresChange = function(event) {
			...
			$(this).parents(".idAddressBar").find(".creceiveaddrid_name").html(addressName);
		};
		/**
		 * 订单打印
		 */
		...
		this.init = function() {
			...
			// 绑定回车事件，回车后，光标锁定下一个输入框
			$("body").delegate(".add_input_name,.add_input_amount,.idOrderAdd", "keydown", function(event) {
			...
			//删除收货地址显示事件
			$(".ui-cleaner").live("click",function(){
				$(this).parent().find("input[type='hidden']").val("");
				$(this).addClass("hidden");
				$(this).parent().find(".J-addressSearcher").val("").blur();
			});
