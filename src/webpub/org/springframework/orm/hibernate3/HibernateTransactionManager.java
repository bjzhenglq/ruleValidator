 ...
 * {@link SessionFactoryUtils} and {@link HibernateTemplate} are aware of
 * thread-bound Sessions and participate in such transactions automatically.
 * Using either of those or going through
 ...
 * or preferably create the SessionFactory with {@link LocalSessionFactoryBean}
 * and a local DataSource (which will be autodetected by this transaction
 * manager).
 * 
 * <p>
 * JTA (usually through
 ...
 * somewhat container-specific due to the JTA TransactionManager lookup,
 * required for proper transactional handling of the SessionFactory-level
 * read-write cache.
 * 
 * <p>
 ...
 * {@link org.springframework.transaction.jta.JtaTransactionManager}'s javadoc
 * for details.
 * 
 * <p>
...
public class HibernateTransactionManager extends
	 ...
	 * will be auto-detected: You can still explictly specify the DataSource,
	 ...
	 * <code>setDataSource</code>. Default is "true".
	 * <p>
	 ...
	 * transaction begin will fail if the <code>getCurrentSession()</code> call
	 * fails.
	 * <p>
	 * This mode will typically be used in combination with a custom Hibernate
	 * {@link org.hibernate.context.CurrentSessionContext} implementation that
	 * stores Sessions in a place other than Spring's
	 ...
	 * receive a <code>clear()</code> call (on rollback) or a
	 * <code>disconnect()</code> call (on transaction completion) in such a
	 * scenario; this is rather left up to a custom CurrentSessionContext
	 ...
	 * making flushed state visible to <code>beforeCommit</code> callbacks of
	 * registered
	...
	/**
	 * Return the current Hibernate entity interceptor, or <code>null</code> if
	 * none. Resolves an entity interceptor bean name via the bean factory, if
	 * necessary.
	 * 
	 ...
	 * is thrown on flush, overriding Hibernate's default SQLException
	 * translation (which is based on Hibernate's Dialect for a specific target
	 * database).
	 * 
	...
	/**
	 * Return the JDBC exception translator for this transaction manager, if
	 * any.
	 */
	...
	public void afterPropertiesSet() {
		...
		if (this.autodetectDataSource && getDataSource() == null) {
			DataSource sfds = SessionFactoryUtils
		...
		if (sessionHolder != null) {
			if (logger.isDebugEnabled()) {
				logger.debug("Found thread-bound Session ["
						+ SessionFactoryUtils.toString(sessionHolder
								.getSession()) + "] for Hibernate transaction");
			}
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			if (txObject.getSessionHolder() == null
				...
				Session newSession = (entityInterceptor != null ? getSessionFactory()
						.openSession(entityInterceptor) : getSessionFactory()
						.openSession());
				if (logger.isDebugEnabled()) {
					logger.debug("Opened new Session ["
							+ SessionFactoryUtils.toString(newSession)
			...
			if (definition.isReadOnly() && txObject.isNewSession()) {
				// Just set to NEVER in case of a new Session for this
				// transaction.
	...
	protected void doCommit(DefaultTransactionStatus status) {
		...
		if (status.isDebug()) {
			logger.debug("Committing Hibernate transaction on Session ["
					+ SessionFactoryUtils.toString(txObject.getSessionHolder()
							.getSession()) + "]");
		}
		try {
	...
	protected void doCleanupAfterCompletion(Object transaction) {
		...
		if (txObject.isNewSession()) {
			...
			if (txObject.getSessionHolder().getPreviousFlushMode() != null) {
				session.setFlushMode(txObject.getSessionHolder()
	 ...
	 * <p>
	 * Default implementation checks the Session's connection release mode to be
	 * "on_close". Unfortunately, this requires casting to SessionImpl, as of
	 * Hibernate 3.1. If that cast doesn't work, we'll simply assume we're safe
	...
	protected boolean isSameConnectionForEntireSession(Session session) {
		...
		ConnectionReleaseMode releaseMode = ((SessionImpl) session)
	...
	protected DataAccessException convertHibernateAccessException(
		...
		if (getJdbcExceptionTranslator() != null && ex instanceof JDBCException) {
			...
			return convertJdbcAccessException((GenericJDBCException) ex,
	 ...
	 * from the <code>org.springframework.dao</code> hierarchy, using the given
	...
	protected DataAccessException convertJdbcAccessException(JDBCException ex,
		...
		return translator.translate("Hibernate flushing: " + ex.getMessage(),
				ex.getSQL(), ex.getSQLException());
	}
	...
	protected synchronized SQLExceptionTranslator getDefaultJdbcExceptionTranslator() {
		if (this.defaultJdbcExceptionTranslator == null) {
			if (getDataSource() != null) {
				this.defaultJdbcExceptionTranslator = new SQLErrorCodeSQLExceptionTranslator(
						getDataSource());
			} else {
	 ...
	 */
	private class HibernateTransactionObject extends
		...
		public boolean isRollbackOnly() {
			return this.sessionHolder.isRollbackOnly()
					|| (hasConnectionHolder() && getConnectionHolder()
							.isRollbackOnly());
		}
