 ...
 * SessionFactory can then be passed to Hibernate-based DAOs via dependency
 * injection.
 * 
 * <p>
 ...
 * for transaction demarcation, with the latter only necessary for transactions
 ...
 * <code>getCurrentSession()</code> will also seamlessly work with a
 * request-scoped Session managed by
 * {@link org.springframework.orm.hibernate3.support.OpenSessionInViewFilter} /
 * {@link org.springframework.orm.hibernate3.support.OpenSessionInViewInterceptor}.
 * 
 * <p>
 * <b>Requires Hibernate 3.2 or later; tested with 3.3, 3.5 and 3.6.</b> Note
 * that this factory will use "on_close" as default Hibernate connection release
 * mode, unless in the case of a "jtaTransactionManager" specified, for the
 * reason that this is appropriate for most Spring-based applications (in
 * particular when using Spring's HibernateTransactionManager).
 * 
...
public class LocalSessionFactoryBean extends AbstractSessionFactoryBean
	...
	private static final ThreadLocal<org.hibernate.cache.CacheProvider> configTimeCacheProviderHolder = new ThreadLocal<org.hibernate.cache.CacheProvider>();
	...
	/**
	 * Return the RegionFactory for the currently configured Hibernate
	 * SessionFactory, to be used by LocalRegionFactoryProxy.
	 * <p>
	 * This instance will be set before initialization of the corresponding
	 * SessionFactory, and reset immediately afterwards. It is thus only
	 * available during configuration.
	...
	/**
	 * Return the CacheProvider for the currently configured Hibernate
	 * SessionFactory, to be used by LocalCacheProviderProxy.
	 * <p>
	 * This instance will be set before initialization of the corresponding
	 * SessionFactory, and reset immediately afterwards. It is thus only
	 * available during configuration.
	 ...
	 * TransactionManagerLookup. Allows for using a Spring-managed JTA
	...
	/**
	 * Set the Hibernate RegionFactory to use for the SessionFactory. Allows for
	 * using a Spring-managed RegionFactory instance.
	 * <p>
	 * As of Hibernate 3.3, this is the preferred mechanism for configuring
	 * caches, superseding the {@link #setCacheProvider CacheProvider SPI}. For
	 * Hibernate 3.2 compatibility purposes, the accepted reference is of type
	...
	/**
	 * Set the Hibernate CacheProvider to use for the SessionFactory. Allows for
	 * using a Spring-managed CacheProvider instance.
	 * <p>
	 * Note: If this is set, the Hibernate settings should not define a cache
	 * provider to avoid meaningless double configuration.
	 * 
	 * @deprecated as of Spring 3.0, following Hibernate 3.3's deprecation of
	 *             the CacheProvider SPI
	 * @see #setCacheRegionFactory
	 */
	@Deprecated
	 ...
	 * entities). This configuration setting corresponds to the
	 * &lt;class-cache&gt; entry in the "hibernate.cfg.xml" configuration
	 * format.
	 * <p>
	 * For example:
	 * 
	 ...
	 * keys and listener objects as values. Instead of a single listener object,
	...
	protected SessionFactory buildSessionFactory() throws Exception {
		...
		try {
			if (isExposeTransactionAwareSessionFactory()) {
				...
				// Can be overridden by a custom value for the corresponding
				// Hibernate property.
			...
			if (this.jtaTransactionManager != null) {
				...
				// property.
				config.setProperty(Environment.TRANSACTION_STRATEGY,
			...
			if (this.typeDefinitions != null) {
				...
				// Use reflection for compatibility with both Hibernate 3.3 and
				// 3.5:
				// the returned Mappings object changed from a class to an
				// interface.
				Method createMappings = Configuration.class
						.getMethod("createMappings");
				Method addTypeDef = createMappings.getReturnType().getMethod(
						"addTypeDef", String.class, String.class,
						Properties.class);
				Object mappings = ReflectionUtils.invokeMethod(createMappings,
						config);
				for (TypeDefinitionBean typeDef : this.typeDefinitions) {
					ReflectionUtils.invokeMethod(addTypeDef, mappings,
							typeDef.getTypeName(), typeDef.getTypeClass(),
			...
			if (dataSource != null) {
				...
				if (isUseTransactionAwareDataSource()
					...
					providerClass = TransactionAwareDataSourceConnectionProvider.class;
						...
						.getProperty(Environment.TRANSACTION_MANAGER_STRATEGY) != null) {
			...
			if (this.cacheRegionFactory != null) {
				...
				config.setProperty(Environment.CACHE_PROVIDER,
						LocalCacheProviderProxy.class.getName());
			}
			...
			// Tell Hibernate to eagerly compile the mappings that we
			// registered,
			// for availability of the mapping information in further
			// processing.
			...
			if (this.entityCacheStrategies != null) {
				...
				for (Enumeration<?> classNames = this.entityCacheStrategies
					...
					String[] strategyAndRegion = StringUtils
							.commaDelimitedListToStringArray(this.entityCacheStrategies
									.getProperty(className));
					if (strategyAndRegion.length > 1) {
						// method signature declares return type as
						// Configuration on Hibernate 3.6
						// but as void on Hibernate 3.3 and 3.5
						Method setCacheConcurrencyStrategy = Configuration.class
								.getMethod("setCacheConcurrencyStrategy",
										String.class, String.class,
										String.class);
						ReflectionUtils.invokeMethod(
								setCacheConcurrencyStrategy, config, className,
			...
			if (this.collectionCacheStrategies != null) {
				...
				for (Enumeration<?> collRoles = this.collectionCacheStrategies
					...
					String[] strategyAndRegion = StringUtils
							.commaDelimitedListToStringArray(this.collectionCacheStrategies
	 ...
	 * on the Configuration object after this FactoryBean registered its
	 * specified mappings.
	 * <p>
	 * Invoked <i>before</i> the <code>Configuration.buildMappings()</code>
	 * call, so that it can still extend and modify the mapping information.
	 * 
	 * @param config
	 ...
	 * <p>
	 * Invoked <i>after</i> the <code>Configuration.buildMappings()</code> call,
	 * so that it can operate on the completed and fully parsed mapping
	 * information.
	 * 
	 * @param config
	 *            the current Configuration object
	...
	public void updateDatabaseSchema() throws DataAccessException {
		...
		try {
			...
			final Dialect dialect = ((SessionFactoryImplementor) sessionFactory)
					.getDialect();
	...
	public void dropDatabaseSchema() throws DataAccessException {
		...
		hibernateTemplate.execute(new HibernateCallback<Object>() {
			...
			public Object doInHibernate(Session session)
				...
				@SuppressWarnings("deprecation")
				Connection con = session.connection();
				String[] sql = getConfiguration().generateDropSchemaScript(
						dialect);
				executeSchemaScript(con, sql);
				return null;
			}
